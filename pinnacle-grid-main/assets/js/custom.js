...
function sendWhatsApp(service, page) {
  var message = 'Hi Pinnacle Grid, I visited your website and I am interested in ' + service + '. I found this on your ' + page + ' page. Please share complete details, pricing, and contact me as soon as possible.';
  var url = 'https://wa.me/919100305750?text=' + encodeURIComponent(message);
  window.open(url, '_blank');
}

// ======= Web3Forms AJAX Submission & Redirect Fix =======
document.addEventListener("DOMContentLoaded", function () {
    ...
    form.addEventListener("submit", function (e) {
      ...
      .then(async (response) => {
        ...
        if (response.status == 200) {
          // Success: Manual redirect to the thank-you page defined in the hidden input
          const redirectInput = form.querySelector('input[name="redirect"]');
          let redirectUrl = "thanks.html"; // Default fallback
          
          if (redirectInput && redirectInput.value) {
            // Use the absolute URL if provided, but prioritize relative for local testing if it's on the same origin
            redirectUrl = redirectInput.value;
            
            // If the absolute URL domain matches the current origin, we can just use the path
            try {
